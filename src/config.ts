...
export type PolicyMode = "" | "jev" | "dumb" | "numeric" | "stance";
...
export function resolvePolicy(e: { POLICY?: string }): PolicyMode {
  ...
  if (!v) return "";
  if (v === "jev" || v === "dumb" || v === "numeric" || v === "stance") return v;
  throw new Error("POLICY must be jev, dumb, numeric or stance");
}
...
export const config = {
  ...
  jevModelId,
  jevUsdPerMTok: 0.042,
  /** "" = caminho legado (o MODEL decide). jev | dumb | numeric | stance = caminho da fusao. */
